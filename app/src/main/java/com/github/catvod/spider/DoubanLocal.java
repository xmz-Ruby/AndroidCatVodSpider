...
public class DoubanLocal extends Spider {
    ...
    public void init(Context context, String extend) throws Exception {
        this.extend = extend;
        SpiderDebug.log("DoubanLocal init completed");
        // downloadFile logic moved to Init.init() for async execution
    }

    @Override
    public String homeContent(boolean filter) throws Exception {
        SpiderDebug.log("homeContent 开始执行");

        // 简化逻辑，不再触发异步下载，直接构建页面内容
        ...
        // 注释掉频繁下载逻辑，已移至Init类异步执行
        SpiderDebug.log("homeContent 完成页面构建");
